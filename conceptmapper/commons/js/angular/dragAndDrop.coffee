...
startShowDropAllowedState = (element)->
  showDropNotAllowedState(element) unless element.hasClass("dontShowDropNotAllowed")
...
showDropNotAllowedState = (element) ->
  element.addClass(dropAllowedStateClass) unless element.hasClass("ui-dialog") || element.hasClass("dontShowDropNotAllowed")
...
draggableDirective = () ->
  ...
  link: (scope, element, attrs)->
    ...
    if (dragOptions.revert && dragOptions.revert[0] == "#")
      revertHomeElementId = dragOptions.revert
      dragOptions.revert = (dropped)->
        if (!dropped)
          revertHomeElement = $(revertHomeElementId)
          revertHomeElementOffset = revertHomeElement.offset()
          currentElementOffset = element.offset();
          currentElementPosition = element.position();
          newOriginalPosition = {
            top: revertHomeElementOffset.top - currentElementOffset.top + currentElementPosition.top,
            left: revertHomeElementOffset.left - currentElementOffset.left + currentElementPosition.left
          };
          draggableData = element.data("ui-draggable")
#          console.log("changing from #{JSON.stringify(draggableData.originalPosition)} to #{JSON.stringify(newOriginalPosition)}")
          draggableData.originalPosition = newOriginalPosition;
        !dropped
    originalZIndex = null
    ...
    dragOptions.stop = (event, ui)=>
      ...
      draggableInformation = collectDraggableInformation(dropElement, element)
      draggableInformation.dropped = dropped
