  ...
  elementFinder.findElement = function findElement(loc, container) {
    ...
    if (loc === "body") {
      ...
      if (! el) {
        ...
        // as a fallback, don't throw an error, but return document.body
        return document.body;
      }
      if (rest) {
        try {
...
define('ui',["require", "jquery", "util", "session", "templates", "templating", "linkify", "peers", "windowing", "tinycolor", "elementFinder", "visibilityApi"], function (require, $, util, session, templates, templating, linkify, peers, windowing, tinycolor, elementFinder, visibilityApi) {
  ...
  ui.prepareUI = function () {
    ...
    TogetherJS.config.track("disableShareButton", function (hide, previous) {
        if (hide && ! previous) {
    ...
    undock: function () {
      ...
      this.dockElement.animateDockExit().promise().then((function () {
        try {
            ...
            // if the dockElement has already been removed, we may trigger an exception here. catch and do nothing.
        }
      }).bind(this));
    },
...
define('cursor',["jquery", "ui", "util", "session", "elementFinder", "tinycolor", "eventMaker", "peers", "templating"], function ($, ui, util, session, elementFinder, tinycolor, eventMaker, peers, templating) {
    ...
    updatePosition: function (pos) {
      ...
      if (pos.element) {
        try {
            ...
            // there was a problem with finding the element, so simply don't update the position
            return;
        }
  ...
  function mousemove(event) {
    ...
    try {
        ...
        // an error happened during elementLocation, so just ignore and do nothing
    }
  }
  ...
  function _scrollRefresh() {
    try {
        ...
        // something went wrong (probably in elementFinder.elementByPixel), doing nothing
    }
  }
  ...
  function documentClick(event) {
    ...
    setTimeout(function () {
      ...
      try {
          ...
          // something went wrong during element locating, fallback:
          location = element;
      }
  ...
  session.hub.on("cursor-click", function (pos) {
    ...
    try {
        ...
        // an error happened (propably during elementFinder.findElement), setting top and left to my buest guess
        left = pos.offsetX;
        top = pos.offsetY;
    }
