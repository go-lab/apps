...
//        printf('Point.draw: ' + pt);
    ...
    Point.prototype.subtract = function(pt2) {
        var pt = this;

        return new geo.Point(pt.x() - pt2.x(), pt.y() - pt2.y());
    }
    ...
    /**
     *  Create a line from two points or slope (m) and intercept (b).
     *
     *  @constructor
        ...
        case 1:
            ...
            if (opts.p && opts.q) {
                ...
                if (opts.m && opts.b) {
                    ...
                    if (m === null) {
                        ln._p = new Point(b, 10);
                        ln._q = new Point(b, 15);
                    } else {
                        ln._p = new Point(10, m*10);
                        ln._q = new Point(15, m*15);
                    }
                } else {
    ...
    Line.prototype.parallel = function(pt) {
        console.log('   Line.parallel not tested');
        var ln = this;
        var m = ln.slope();

        if (m === null)
            return new Line(pt.x(), pt.y(), pt.x(), pt.y()+10);

        var b = -(m * pt.x()) + pt.y();

        return new Line({ m: m, b: b});
    }
...
//        printf('Line.draw: ' + ln);
     ...
     */
    Line.prototype.rotate = function(center, theta) {
        var ln = this;
        var p2 = ln.p().rotate(center, theta);
        var q2 = ln.q().rotate(center, theta);
    ...
    Line.prototype.point_at_distance = function(p1, p2, d) {
        ...
        if (!rval)
            rval = p2.closest(ln.points_at_distance(p1,d));

        if (rval.y() > 1000) {
            console.log('Line.point_at_distance ');
...
*/
     ...
     *  @return {Array}	points New points on this line that is d away from p
     */
    ...
    Line.prototype.angle = function(ln2) {
        var ln1 = this;
        var theta1 = Math.atan2(ln1.y1() - ln1.y2(),
                                ln1.x1() - ln1.x2());
        var theta2 = Math.atan2(ln2.y1() - ln2.y2(),
                                ln2.x1() - ln2.x2());
    ...
    { var F = function() {}; F.prototype = Line.prototype; Ray.prototype = new F(); Ray.prototype.constructor = Ray; Ray.superclass = Line.prototype; if (Line.prototype.constructor == Object.prototype.constructor) Line.prototype.constructor = Line; };;
...
//        printf('  intersection ' + pt);
    ...
    Ray.prototype.draw = function(style) {
        ...
        if (pts.length !== 1) {
            console.log('Ray.points not one ' + pts.length);
            console.log('  ' + r);
            console.log('  ' + rect);
            for (var pt, i=0; pt=pts[i], i<pts.length; i++) {
                console.log(i + ' ' + pt);
...
//            ln.draw();
    ...
    { var F = function() {}; F.prototype = Ray.prototype; Segment.prototype = new F(); Segment.prototype.constructor = Segment; Segment.superclass = Ray.prototype; if (Ray.prototype.constructor == Object.prototype.constructor) Ray.prototype.constructor = Ray; };;
...
//        printf('Segment.draw: ' + s);
    ...
    Circle.prototype.angle = function(pt) {
        ...
        return Math.atan2(pt.y() - c.cy(), pt.x() - c.cx());
    }
...
*/
    ...
    var Triangle = geo.Triangle = function(a, b, c) {
        var tr = this;
        ...
        var angle = new geo.Line(a, b).angle(new geo.Line(c,b));

        return tr;
    }

    Triangle.prototype.A = function() { return this._A; };
    Triangle.prototype.B = function() { return this._B; };
    Triangle.prototype.C = function() { return this._C; };
    ...
    var Polygon = geo.Polygon = function() {
        var p = this;
        var len = arguments.length;
    ...
    Polygon.prototype.points = function() { return this._points; };
    ...
    Polygon.prototype.area = function() {
        ...
        pts.push(pts[0]); // Close polygon, first and last are the same
    ...
    /**
     *  Convert to an acute angle (0 > theta > 90).
     *
     *  @param {number}		Radians.
     *  @returns {number}	Radians.
     */
    geo.acute_angle = function(rads) {
        if (rads < 0)
            return geo.acute_angle(rads + Math.PI);
        if (rads > Math.PI / 2)
            return geo.acute_angle(rads - Math.PI / 4);
        return rads;
    }
    ...
    geo.collinear = function(p, q, r) {
        { if (!(p instanceof Point)) { console.log('assert: ' + "geometry.src" + ': ' + 1519 + ' ' + "p" + ' not a ' + "Point"); return null;} };
        { if (!(q instanceof Point)) { console.log('assert: ' + "geometry.src" + ': ' + 1520 + ' ' + "q" + ' not a ' + "Point"); return null;} };
        { if (!(r instanceof Point)) { console.log('assert: ' + "geometry.src" + ': ' + 1521 + ' ' + "r" + ' not a ' + "Point"); return null;} };
        var px = p.x();
        var py = p.y();
        var qx = q.x();
        var qy = q.y();
    ...
    /**
     *  Succeed when point a is between p and q (on the same line).
     */
...
/*
        printf(' ap ' + ap.toFixed(2) + ' aq ' + aq.toFixed(2) + ' pq ' + pq.toFixed(2));
        printf(' ap + aq ' + (ap+aq));
*/

        var diff = Math.abs(ap+aq-pq);

        return diff < geo.epsilon;
