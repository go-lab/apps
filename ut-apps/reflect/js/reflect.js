...
//    var WEBSERVICE_URL = "http://localhost:3090/analytics/reflect";
    ...
    console.log('========== loading tools/reflect/js/reflect.js');
    ...
    reflect.configure = function(spec, callback) {
        ...
        reflect.set_context();
        console.log('configure [Enter] ' + reflect.ils_context);
        if (reflect.ils_context === 'ils' || reflect.ils_context === 'graasp') {
            ...
            mh.getILSStructure(function(err, result) {
                if (err) {
                    console.log('mh.getILSStructure');
                    console.log('ERROR ' + JSON.stringify(err,null,4));
                    return callback();
                } else {
                    reflect.ils_structure = result;
                    phase_names = reflect.phases_from_ils_structure(result);
                    console.log('configure [In ILS]');
                    console.log('  ILS PHASE NAMES ' + phase_names);
                    load_all(phase_names, callback);
                }
            });
        } else {
            phase_names = reflect.default_phases();
            console.log('configure [Standalone]');
            console.log('   default phases ' + phase_names);
            load_all(phase_names, callback);
        }
        ...
        function load_all(phase_names, callback) {
            ...
            reflect.load_configuration(function() {
                ...
                console.log('  [ils_context] = ' + reflect.ils_context);
                if (reflect.ils_context === 'ils')
                    reflect.configuration.show_configuration = false;
                console.log('  [show_configuration] = ' + reflect.configuration.show_configuration);
                console.log('load_all [Configuration] ');
    ...
    reflect.update_phase_names_for_configuration = function(names) {
        ...
        console.log('update_phase_names_for_configuration [Enter]');
        ...
        console.log('  update_phase_names_for_configuration [Phases]');
        console.log(JSON.stringify(reflect.configuration.model.phases,null,4));
        console.log('update_phase_names_for_configuration [Done]');
    };
    ...
    reflect.start = function(spec) {
        ...
        console.log('  configuration ' + reflect.configuration_id);
        console.log('  resource_     ' + reflect.resource_id);
        reflect.model.summarize();
...
//        request_data();
    ...
    ng_reflect.controller("reflect_controller", function($scope) {
        reflect.scope = $scope;
        console.log('[Reflect] .reflect controller');
    ...
    function request_data() {
        console.log('request_data [Enter]');
        var scope = reflect.scope;
        ...
        function handle_response(response) {
            ...
            if (response.error) {
                console.log('REFLECT ERROR ' + console.log(JSON.stringify(response.error,null,4)));
