...
(function() {
    ...
    edt.load_configuration = function(callback) {
        ...
        sh.readLatestResource('configuration', function(error, resource) {
            if (error) {
                console.log('          [EDT] readLatestResource failed');
                console.log('--------- CB load_configuration');
                return callback();
            }

            if (resource === null) {
                console.log('          creating empty configuration');
                sh.createResource({
                    ...
                    console.log('          CB createResource');
                    if (error) {
                        console.log('sh.createResource error ' + JSON.stringify(error));
                        console.log('(5) ' + edt.configuration.auto_save);
                        console.log('--------- CB load_configuration');
                        return callback();
                    }
