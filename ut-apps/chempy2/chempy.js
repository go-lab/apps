...
(function() {
    ...
    printf('========== loading tools/chempy2/js/chempy.js');
...
*/
    ...
    chempy.configure = function(spec, callback) {
        ...
        sh.listResourceMetaDatas(function(error,meta) {
            for (var i=0; i<meta.length; i++) {
                ...
                printf('resource ' + entry.metadata.target.objectType);
            ...
            if (chempy.configuration_resource_id === null) {
                sh.createResource({
                    ...
                    if (error) {
                        ...
                        printf('(5) ' + chempy.configuration.auto_save);
                        return callback();
                    }
    ...
    ng_chempy.controller("chempy_controller", function($scope) {
        ...
        $scope.check_answer = function(event, eq) {
            ...
            if (!eq.correct && eq.attempts >= eq.tries) {
                printf('eq.correct ' + eq.correct);
                printf('eq.attempts ' + eq.attempts);
                printf('eq.tries ' + eq.tries);
        ...
        $scope.edit_configuration = function() {
            ...
            $(id).dialog({ title: title,
                           ...
                           draggable: true,
                           create: function() {
                               printf('setting gadgetResize');
                               setTimeout(function() {
                                   printf('calling gadgetResize');
    ...
    chempy.check_answer = function(eq) {
        ...
        printf('  answer parts   ' + JSON.stringify(answer_parts));
        printf('  text parts   ' + JSON.stringify(text_parts));
...
'          </div>';
    ...
    chempy.reset_configuration = function(atts) {
        ...
        var answer = use_default(atts.answer, conf.answer);
        var help_button = use_default(atts.help_button, conf.help_button);
        var tries = use_default(atts.tries, conf.tries);
