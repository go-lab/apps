...
function visualizeFromJSON(network){
    ...
    if ((network != "") && (network.data !== "undefined") && (network.data.nodes !== "undefined") && (network.data.nodes.length > 0)) {
        ...
        if (framework.initialize(network)) {
...
function VisualizationFramework(){
            ...
            if(nodeset!=undefined || nodeset!=null){
                ...
                for(var i=0;i<nodeset.length;i++){
                    ...
                    if(transferObjectIndex!=-1){
                        ...
                        var transferObject = {id:currentNode.id,label:currentNode.label,x:currentNode.x,y:currentNode.y,fixed:true};
    ...
    function initializeCanvas(){
        ...
        gZoom =d3.behavior.zoom().scale(scale).translate(translate).scaleExtent([1, 50]).on('zoom', function(){adjustZoom();adjustExplorationContext();});

        canvas = d3.select('.content-body')
            .append('svg')
            .attr('id','canvas')
            .attr('viewBox', '0 0 ' + canvasWidth + ' ' + canvasHeight)
            .attr('preserveAspectRatio', 'xMidYMid meet')
            .style("height", window.innerHeight - titleHeight)
            .call(gZoom)
            .append('svg:g');
    ...
    function displayNetworkInfo(){
        ...
        }else{ customoptions.append('p').text('Unable To Detect Network Direction.');}
    ...
    function updateNodeMeasureList(){
        d3.select('#nodehighlight').append('option').attr('value','default').text('Default').attr('selected',function(){if(nodeMeasureContext=='default') return 'selected'});
        nodeMeasures.forEach(function(measure){
            d3.select('#nodehighlight').append('option').attr('value',measure.property).text(measure.title).attr('selected',function(){if(nodeMeasureContext==measure.property) return 'selected'});
        });
    ...
    function updateEdgeMeasureList(){
        d3.select('#edgehighlight').append('option').attr('value','default').text('Default').attr('selected',function(){if(edgeMeasureContext=='default') return 'selected'});
        edgeMeasures.forEach(function(measure){
            d3.select('#edgehighlight').append('option').attr('value',measure.property).text(measure.title).attr('selected',function(){if(edgeMeasureContext==measure.property) return 'selected'});;
        });
    ...
    function filterNodes(){
        if(nodeFilterValue<1){
            ...
            d3.selectAll(nodeNotation).each(function(){
                d3.select(this).attr('opacity',function(node){ return retrieveNodeAlphaValue(node)});
            });
    ...
    function filterEdges(){
        if(d3.select('#edgefilter').property('value')<1){
            ...
            d3.selectAll(edgeNotation).each(function(){
                d3.select(this).attr('opacity',function(edge){ return retrieveEdgeAlphaValue(edge)});
            });
        }
    }
    ...
    function highlightNodeMeasure(){
        d3.selectAll(nodeNotation).transition().attr('d', d3.svg.symbol().size(function(node) {return retrieveNodeSize(node);}).type(function(node){return retrieveNodeSymbol(node)}));
    }
    //function for highglighting the edge measures
    function highlightEdgeMeasure(){
        d3.selectAll(edgeNotation).transition().attr('stroke-width',function(edge){return retrieveEdgeSize(edge)})
    }

    //function for updating the node property list
    function updateNodePropertyList(){
        d3.select("#nodeproperty").append('option').attr('value','default').text('Default').attr('selected',function(){if(nodePropertyContext=='default')return 'selected'});

        extractProperties(nodeproperties).forEach(function(property){
            d3.select("#nodeproperty").append('option').attr('value',property).text(property.toString().toUpperCase()).attr('selected',function(){if(nodePropertyContext==property) return 'selected'});
        });
    ...
    function updateEdgePropertyList(){
        d3.select("#edgeproperty").append('option').attr('value','default').text('Default').attr('selected',function(){if(edgePropertyContext=='default')return 'selected'});
        extractProperties(edgeproperties).forEach(function(property){
            d3.select("#edgeproperty").append('option').attr('value',property).text(property.toString().toUpperCase()).attr('selected',function(){if(edgePropertyContext==property) return 'selected'});
        });
    ...
    function updateEdgeColors(){
        d3.selectAll(edgeNotation).each(function(){
            d3.select(this).attr("stroke",function(d){return retrieveEdgeColor(d);})
        });
    };
    ...
    function updateNodeColors(){
        d3.selectAll(nodeNotation).each(function(){
            d3.select(this).attr("fill",function(d){return retrieveNodeColor(d);})
        })
    ...
    function displaySearch(){
        ...
        customoptions.append('select').attr('id','scope').on('click',function(){updateScope(d3.select(this).property('value'))});
        d3.select('#scope').append('option').attr('value',0).text('None').attr('selected',function(){if(scopeContext==1)return 'selected'})
        d3.select('#scope').append('option').attr('value',1).text('1.0x').attr('selected',function(){if(scopeContext==1)return 'selected'})
        d3.select('#scope').append('option').attr('value',2).text('1.5x').attr('selected',function(){if(scopeContext==2)return 'selected'})
        d3.select('#scope').append('option').attr('value',3).text('2.0x').attr('selected',function(){if(scopeContext==3)return 'selected'})
        d3.select('#scope').append('option').attr('value',4).text('Trail').attr('selected',function(){if(scopeContext==4)return 'selected'})
        customoptions.append('hr')
        customoptions.append('p').text('Search');
        customoptions.append("datalist").attr('id','suggestions').selectAll("option").data(nodeset.map(function (node){return node.label})).enter().append("option").attr("value", function(node) {return node;} );
        customoptions.append('input').attr('id', 'searchfield').attr('type', 'text').attr("list","suggestions").style('align','center').on('keydown', function() { if(d3.event.keyCode==13){searchNode(d3.select(this).property("value").toString().trim());}else if(d3.event.keyCode==27){cleanSearch();} });
        customoptions.append('input').attr('id', 'searchbutton').attr('type', 'button').attr('value', 'Search').on('click', function() {searchNode(d3.select('#searchfield').property("value").toString().trim());});
        customoptions.append('input').attr('id', 'cleanbutton').attr('type', 'button').attr('value', 'Clean').on('click', function() {cleanSearch(); });
        ...
        if(keyword===""){
            ...
            d3.selectAll(nodeNotation).each(function(d){
                if(d.label.toLocaleLowerCase().indexOf(keyword.toLocaleLowerCase())!=-1){
                    ...
                    d3.select(this).attr("d", d3.svg.symbol().size(function(d) {return retrieveNodeSize(d)*100;}).type(function(d) { return retrieveNodeSymbol(d);})).transition().attr("opacity",1.0).transition().attr("d",originalPath);
                }
            });
    ...
    function displayTimeNavigation(){
        ...
        customoptions.append('input').attr('id', 'backward').attr('type', 'button').attr('value', '<').on('click', function() { moveBackwards();});
        ...
        customoptions.append('input').attr('id', 'forward').attr('type', 'button').attr('value', '>').on('click', function() { moveForward();});
        customoptions.append('hr');
        ...
        customoptions.append('input').attr('id','timeslider').attr('type','range').attr('min','0').attr('max',function(){return (datalinks.length-1)}).attr('value',currentTimeFrame).on('change',function(){
        ...
        customoptions.append('input').attr('id', 'frameduration').attr('type', 'text').property('value',duration).style('text-align','center').on('keydown', function() { if(d3.event.keyCode==13){updateMovieFrame();} });
        customoptions.append('input').attr('id', 'framedurationbutton').attr('type', 'button').attr('value', 'Apply').on('click', function() {updateMovieFrame();});
        customoptions.append('hr');
        customoptions.append('p').text('Visual Stability');
        customoptions.append('input').attr('id', 'vstability').attr('type', 'checkbox').property('checked',visualStability).on('change',function(){updateVisualStability();});
    ...
    this.requestNodeInformation = function(node){
        d3.selectAll(nodeNotation).each(function(d){
            if(d.id == node.id){
                ...
                selection.transition().attr("d", d3.svg.symbol().size(function(){ if(nodeMeasureContext!='default'){ return nodeSizeScale(node[nodeMeasureContext]); } return defaultNodeSize*5; }).type(function(node){return retrieveNodeSymbol(node)}));
                selection.append("svg:title").data([htmlContent]).attr('class','tooltip').text(function(d){return d});
            }
        })
    };
    ...
    this.requestNodeInformationRemoval = function(node){
        d3.selectAll(nodeNotation).each(function(d){
            if(d.id==node.id) {
                var selection = d3.select(this);
                selection.transition().attr("d", d3.svg.symbol().size(function(){ if(nodeMeasureContext!='default'){ return nodeSizeScale(node[nodeMeasureContext]); } return defaultNodeSize; }).type(function(node){return retrieveNodeSymbol(node)}));
